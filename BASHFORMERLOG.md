...
## 2026-08-20 — регрессия от собственной минимизации

Первый прогон на polygon стартовал правильно —

```
bashformer forge: 9.075M params, D=256 L=12 Q=8 KV=2 FFN=768 CTX=256 V=96
corpus: 1284274 bytes; train=1155846 val=128428
step 1 | loss 10.5819 | lr 3.00e-05
final val 3.2473; nans 0
```

— и закончился строкой `bashformer-train: final save failed`. Причина моя:
удаляя `weights/.gitkeep`, я положился на `mkdir -p` в рецептах фикстур, а
форж каталог не создаёт. На smoke-прогоне это стоило двадцати шагов; на
десяти тысячах стоило бы всего прогона.

`ensure_parent_dir` создаёт каталог префикса `--out` **до** начала обучения и
падает сразу, если не может, называя путь и `errno`. Сообщение о неудачном
сохранении тоже теперь содержит путь вместо трёх слов.

Проба: `--out /nonexistent-root/x/model` →
`bashformer-train: cannot create /nonexistent-root/x: No such file or directory`,
до первого шага. `make test` 8/8.

## 2026-08-20 — загрузка 9M весов и что в bash стоит дорого

### Гейт: Bash действительно поднимает девятимиллионное тело

```
bash bashformer.sh --weights weights/smoke.bfw --prompt A --tokens 1 --next-id
-> токен, 840.6 s, RSS ~530 MB
```

Работает — и четырнадцать минут только на чтение 34 MB BFW1. Это стало главным
барьером, потому что платится на каждом запуске процесса.

### Что в bash стоит дорого — замеры, не догадки

Стоимость арифметики почти не зависит от вида операции; платим за количество
термов в выражении:

```
256 умножений                      0.409
64 умножения + 256 сложений        0.435   (факторизация по общим весам - хуже)
256 сложений, без умножений        0.365   (всего на 11% дешевле умножений)
256 сдвигов вместо умножений       0.447   (квантование в степени двойки - хуже)
```

Отсюда мёртвыми оказываются все приёмы «заменить операцию на дешёвую»:
факторизация, степени двойки, SWAR-упаковка. Живой рычаг ровно один — убрать
термы. Время линейно по их числу:

```
256 термов 0.259 · 128 термов 0.130 · 64 термов 0.070 · 32 терма 0.036
```

Значит magnitude pruning даёт кратное ускорение прямо: компилятор выражений уже
выбрасывает нули.

Отдельно проверено, парсит ли bash арифметику заново при каждом вычислении.
Одно и то же выражение в переменной, запечённое в тело функции и записанное
литералом в исходнике: 0.164 / 0.177 / 0.170. Разницы нет — предкомпилировать
некуда, «Rosetta внутри bash» невозможна. Механизм `enable -f lib.so` на этом
bash живой, но загружаемый builtin — это shared library, то есть ровно то, что
запрещает `README.md:16`, и организм превращается в C-программу в bash-костюме.

### Что ускорено

Загрузчик вызывал `bf_parse_int` на каждое значение — 9M вызовов, 22.6 мкс
каждый по замеру. Теперь одна проверка регулярным выражением на всю запись `D`
(24 значения), а диапазон проверяется арифметикой. Шаблон по-прежнему
запрещает ведущие нули, поэтому значения безопасно отдавать арифметике: именно
эта проверка не даёт прочитать данные весов как shell.

```
парсинг, 192000 значений:  4.339 s -> 1.380 s   (3.1x)
загрузка + 1 токен на v0:  0.54 s  -> 0.23 s
```

Красные пробы парсера: `x`, `$(touch /tmp/pwned)`, `007`, `99999999999999` —
все четыре отвергнуты, подстановка команд не выполнилась. `make test` 8/8.

### Открыто

Массивы весов и скомпилированные выражения строятся в два прохода. Если
собирать выражение прямо при чтении записей `D`, отпадает и второй проход, и
сами массивы — около сотни секунд и полгигабайта на девятимиллионном теле.

## 2026-08-20 — профиль загрузчика и вторая правка

Профиль старого загрузчика на девятимиллионном теле:

```
PHASE parse 610s · PHASE vocab 0s · PHASE compile 75s   (всего 11m35s с генерацией)
```

Восемьдесят восемь процентов сидело в разборе значений — ровно там, куда била
правка с одним регулярным выражением на запись. После неё на том же теле:

```
840.6 s -> 258.9 s   (3.25x)
```

Дальше самой дорогой частью осталась проверка диапазона: цикл из 24 сравнений
на запись. То же самое одним арифметическим расширением — 0.910 s против
0.429 s на 20000 записей.

Первая версия этой проверки была НЕВЕРНОЙ и я её выбросил: побитовое ИЛИ
модулей может превысить лимит, хотя каждое слагаемое в пределах
(67108864 | 33554432 = 100663296). Корректная форма сравнивает каждое значение
с границей и объединяет результаты сравнений, а не сами значения.

Пробы границ: 67108865 отвергается, 67108864 принимается, симметрично для
отрицательных. Короткий хвостовой рекорд по-прежнему идёт через цикл.
`make test` 8/8.
